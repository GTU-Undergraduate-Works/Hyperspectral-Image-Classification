...
PATCH_SIZE = parameter.lidar_patch_size
...
train_img = dataset.get_patches(PATCH_SIZE, Train=True, HSI=False, LiDAR=True)
test_img = dataset.get_patches(PATCH_SIZE, Train=False, HSI=False, LiDAR=True)
...
flattened = tf.reshape(conv2, [-1, 11 * 11 * CONV2])
fc1 = fc_layer(flattened, 11 * 11 * CONV2, FC1, scope='fc1', relu=True, dropout=True, batch_norm=True)
...
save_dir = 'WeightsLidar/'

if not os.path.exists(save_dir):
    os.makedirs(save_dir)
save_path = os.path.join(save_dir, 'lidar_cnn_model')
